document.addEventListener("DOMContentLoaded", function() {    
    ...
    function handleKeyPress(event)
    {
        ...
        if(!gameStarted && (event.code === 'Space' || event.key === ' ' || event.key === 'Enter' || event.key === 'Spacebar')){
            startGame();
        }
        else if (gameStarted){
            switch(event.key){
    ...
    function stopGame(){
        ...
        let newInstruction=document.createElement('p');
        newLogo.id='logo';
        newInstruction.id='instruction';
        newLogo.src='logo.jpg';
        newInstruction.textContent='Press Space to Start the Snake Game';
        board.appendChild(newLogo);
        board.appendChild(newInstruction);
